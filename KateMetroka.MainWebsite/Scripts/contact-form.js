$(function () {
	...
	$("a#submit").click(function () {
		...
		$.ajax({
			...
			url: "home/contact",
			dataType: "json",
			data: {name: name, emailAddress: email, phoneNumber: phone, message: stripHTML},
			success: function () {
				$('.error').fadeOut(0);
				...
				if (fl) { $('#new_message').fadeIn(); } else
				{
					...
					.fadeIn(1500, function () {
						$('#new_message').append("<br><br><a href='javascript:' id='back' class='link1' onClick='click_back()' >back</a>");
					});
				}
			},
			error: function (jqXHR, textStatus, errorThrown) {
				...
				if (fl) { $('#new_message').fadeIn(); } else
				{
					fl = true
					jQuery('#contact_form').append('<div id="new_message"></div>')
					$('#new_message').append("<div class='download-box'><p class='error'>Error submitting contact form!</p></div>");
					$('#new_message').append("<br><label for='message'><span class='font1'>We apologize for the inconvience, please use the contact information to the right to reach us while we fix this issue.</span></label>")
					.fadeOut(0)
					.fadeIn(1500, function () {
						$('#new_message').append("<br><br><a href='javascript:' id='back' class='link1' onClick='click_back()' >back</a>");
					});
